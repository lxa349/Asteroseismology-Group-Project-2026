...
class power_spectrum:
    ...
    def __init__(self,star_mass, star_radius, star_teff, sun_granulation_tau, sun_granulation_sigma,  sun_facule_tau = 1, sun_facule_sigma = 1, v_nq = 25):
        ...
        self.freq_powerspectrum_uHz = np.linspace(0.1, 100000, 100000) #range of frequencies where power spectrum is evaluated
        self.angular_degree = np.linspace(0,0,1) #List of angular degrees l for oscillations :: FOR TESTING I HAVE REDUCED THIS TO 0
...
def legacy_data(freq_powerspectrum_uHz, comparison_star_KIC_ID): #THIS IS TO TEST PLOTS AGAINST THE LUND DATA
    starTable = ascii.read('/home/isaac/Documents/Asteroseismology Project/fig6.mrt', format='mrt')
    nameList = []
    freqList = []
    ampList = []
    widthList = []
    for i in range(0, len(starTable)):
        if starTable["l"][i] == 0:
            nameList.append(starTable["KIC"][i])
            freqList.append(float(starTable["Freq"][i]))
            ampList.append(float(starTable["Amp"][i]))
            widthList.append(float(starTable["Width"][i]))
    
    comparison_freqList = []
    comparison_ampList = []
    comparison_widthList = []
    for j in range(0, len(nameList)):
        if nameList[j] == comparison_star_KIC_ID:
            comparison_freqList.append(freqList[j])
            comparison_ampList.append(ampList[j])
            comparison_widthList.append(widthList[j])
     
    legacy = np.zeros(len(freq_powerspectrum_uHz))
    for count in range(0, len(comparison_freqList)):
        legacy += comparison_ampList[count] * calc_lorentz(freq_powerspectrum_uHz, comparison_freqList[count], comparison_widthList[count]/2)
    return legacy

def plot_oscillation_comparison(freq_powerspectrum_uHz, oscillation_component, star_nu_max, lund_nu_max, star_name, comparison_star_KIC_ID, comparison_data):
    plt.plot(freq_powerspectrum_uHz, oscillation_component, color = "blue", label = "p-mode oscillations")
    plt.plot(freq_powerspectrum_uHz, comparison_data, color = "red", label = "Legacy data (Lund 2017)")
    # plt.axvline(max_freq(test_star.mass_star, test_star.r_star, test_star.teff_star) * 3100, ls='--', color='r', label = "nu_max")
    plt.axvline(star_nu_max, ls='--', color='b', label = f"simulated nu_max {star_nu_max:.2f}")
    plt.axvline(lund_nu_max, ls='--', color='r', label = f"Lund nu_max {lund_nu_max:.2f}")
    plt.xlim(star.star_nu_max-1000,star.star_nu_max+1000)
    plt.title(f"{star_name} oscillations Power Spectrum")
    plt.xlabel("Frequency /μHertz")
    plt.ylabel("Power /ppm^2 Hz^-1")
    plt.legend(loc = 'best', fontsize = 8)
    plt.show()

def plot_psd(freq_powerspectrum_uHz, oscillation_component, granulation_component, facule_component, star_nu_max, star_name, v_nq, model):
    total_background = facule_component + granulation_component  + oscillation_component #adds both background components and oscillation
    total_background = total_background * (star.nyquist**2) #applies eta term (nyquist)
    facule_component = facule_component * (star.nyquist**2)
    granulation_component = granulation_component* (star.nyquist**2)
...
"""LIST OF USUABLE KIC IDS FROM LEGACY DATA (Lund 2017)
# for star_name, "KIC XXXXXXX"
# must be strings ofc
1435467
2837475
3427720
...
#Some tables use the sun, some do not, be careful

teffTable = Table.read('/home/isaac/Documents/Asteroseismology Project/target_table.tex').to_pandas()
#Mass in solar units is col2, radius in solar units is col5
mrTable = ascii.read('/home/isaac/Documents/Asteroseismology Project/kic_mass_and_radii_lund_II.mrt')
starTable = ascii.read('/home/isaac/Documents/Asteroseismology Project/fig6.mrt', format='mrt')
starList = list(dict.fromkeys(starTable["KIC"]))
starList = starList[:-1]
# print(starList)

for star_index in range(0, len(starList)):
    
    star_mass, star_radius, star_teff = mrTable["col2"][star_index], mrTable["col5"][star_index], teffTable["Teff"][star_index][1:-1]
    
    lund_nu_max = float(teffTable["numax"][star_index][1:-1])
    
    star_name = f"KIC {starList[star_index]}"
    comparison_star_KIC_ID = star_name[4:]

    star = power_spectrum(star_mass, star_radius, star_teff, sun_granulation_tau, sun_granulation_sigma, v_nq = v_nq)

    granulation_component, facule_component = star.theoretical_sigma_tau(model = "Kallinger")
    oscillation_component = star.calc_powder_density()
    comparison_data = legacy_data(star.freq_powerspectrum_uHz, comparison_star_KIC_ID)

    plot_oscillation_comparison(star.freq_powerspectrum_uHz, oscillation_component, star.star_nu_max, lund_nu_max, star_name, comparison_star_KIC_ID, comparison_data)
...
# # plot_psd(star.freq_powerspectrum_uHz, oscillation_component, granulation_component, facule_component, star.star_nu_max, star_name, v_nq ,  model = "Kallinger")
# comparison_data = legacy_data(freq_powerspectrum_uHz, comparison_star_KIC_ID)

# plot_oscillation_comparison(star.freq_powerspectrum_uHz, oscillation_component, star.star_nu_max, star_name, comparison_star_KIC_ID, comparison_data)
