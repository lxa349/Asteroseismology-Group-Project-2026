...
def plot_two_component_psd(freq_powerspectrum_uHz,  granulation_component, facule_component,  star_nu_max, star_name, model):
    ...
    plt.title(f"{star_name} - {model}")
    plt.axvline(star_nu_max, linestyle="--", label=r"$\nu_{\max}$")
    plt.xlim(right=star_nu_max * 1.4)
    plt.ylim(top = max(total_background) * 1.5)
...
def list_import(pic, starlist): #leos woke nonsense
    ...
    for i in pic_tabl2:
        for j in starlist: #forgive the nested for loop, I am mediocre at coding
            if i[0] == j:
                paramater_list.append([i[0], i[1], i[2], i[3]]) #appends: Name, Mass, Radius, Teff of star named
    return(paramater_list)

def single_import(pic, starname): #leos woke nonsense
    pic_list = glob.glob(pic)
    tbl = Table.read(pic_list[0])
    pic_tabl2 = tbl[['StarName', 'Mass', 'Radius', 'Teff']]
...
muh_params = single_import(pic, star_name)

star = power_spectrum(muh_params[1], muh_params[2], muh_params[3], sun_nu_max, sun_teff, sun_granulation_tau,  sun_granulation_sigma, star_name, sun_facule_tau , sun_facule_sigma)
granulation_component_practical, facule_component_practical = star.defined_sigma_tau(star_granultion_sigma_practical, star_granulation_tau_practical, star_facule_sigma_practical, star_facule_tau_practical, model="karoff")
granulation_component_theory, facule_component_theory = star.theoretical_sigma_tau(model=model)
#plot_two_component_psd(star.freq_powerspectrum_uHz, granulation_component_theory, facule_component_theory, star.star_nu_max, star_name,  model = model)
oscillation_component_theory = star.calc_powder_density()

total_theory = granulation_component_theory + facule_component_theory + oscillation_component_theory

plt.figure()
plt.loglog(star.freq_powerspectrum_uHz, total_theory, color = 'r', label='Total theory')
plt.title(f"{star_name} - {model}")
plt.axvline(star.star_nu_max, linestyle="--", label=r"$\nu_{\max}$")
plt.xlim(left = star.star_nu_max * 10**(-3), right = star.star_nu_max * 1.6)
plt.ylim(top = max(total_theory) * 1.6)
