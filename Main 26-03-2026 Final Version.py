...
class power_spectrum:
    ...
    def __init__(self,star_mass, star_radius, star_teff, sun_granulation_tau, sun_granulation_sigma,  sun_facule_tau = 1, sun_facule_sigma = 1, v_nq = 25):
        ...
        self.freq_powerspectrum_uHz = np.linspace(0.1, self.v_nq, 50000) #range of frequencies where power spectrum is evaluated
    ...
    def theoretical_sigma_tau(self, model = "karoff"):
        """
        Calculates the background component with a choice of model. Applies the nyquist cadence term to eaach before returning
        Parameters
        ----------
        ...
        if model == "karoff":
            ...
            granulation_component, facule_component = self.calc_karoff_granulation_facule_model(star_granulation_sigma, star_granulation_tau, star_facule_sigma, star_facule_tau) * (self.nyquist)**2
            ...
            granulation_component =  self.calc_granulation_psd_ball(star_granulation_sigma, star_granulation_tau) * (self.nyquist)**2
    ...
    def calc_karoff_granulation_facule_model(self,  star_granulation_sigma, star_granulation_tau, star_facule_sigma, star_facule_tau):
        """
        Granulation and facule model according to Karoff 2013. Evaluates both components at the frequency linspace. Returns them seperately

        Parameters
        ----------
        star_granulation_sigma : float
            RMS amplitude of the granulation component.
        star_granulation_tau : float
            Characteritic time scale of the granulation component.
        star_facule_sigma : float
            RMS amplitude of the facule component
        star_facule_tau : float
            Characteritic time scale of the facule component.
    ...
    def calc_nu_nl(self, n,l):
        """
        Calculates the frequency   of the p mode oscillation with  radial mode n and angular mode l  
    ...
    def calc_width_nl(self, n, l):
        ...
        #line_width = ((self.alpha * np.log((nu_nl-5*n) / self.star_nu_max)) + np.log(self.width_alpha)) + (np.log(self.delta_width_dip)/(1 + (((2 * np.log((nu_nl-n)/self.nu_dip))/(np.log(self.W_dip/self.star_nu_max)))**2)))
    ...
    def calc_dwarf_suppression_beta_term(self):
        ...
        Returns
        -------
        dwarf_suppression_beta : float
            Suppression term  for hot dwarf planets .
    ...
    def calc_width_parameter(self, a, b, c):
        """
        Calculaates the line width value using the 3 term model
...
def calc_power_amplitude(amp,width):
    """
    Caclulates the unscaled height of a frequency mode
    Now defunct?
...
def calc_lorentz(freq, centroid, FWHM):
    """
    Calculates the value of the lorentzian function defined  by the given FWHM, centroid for each x value in freq array

    Parameters
    ----------
    freq : array
        Array  of frequency to  evaluate the function aat.
    centroid : float
        Centroid of the lorentzian function.
    FWHM : float
        Full width half maxima of the lorentzian.

    Returns
    -------
    x : array
        The  lorentzian functions values at x.
...
def plot_multi_component_psd(freq_powerspectrum_uHz, granulation_component, facule_component, star_nu_max, star_name, v_nq, model):
    ...
    plt.style.use('dark_background') #makes it fit the slides
    
    plt.figure()    
    plt.loglog(freq_powerspectrum_uHz, total_background, label='Total ', color="r")
    plt.loglog(freq_powerspectrum_uHz, facule_component, linestyle = "--", label='Second background component ', color="c")
    plt.loglog(freq_powerspectrum_uHz, granulation_component, linestyle = "--", label='First background component ', color ="yellow")
    
    plt.title(f"Star {star_name} background power spectrum")
    
    plt.axvline(star_nu_max, linestyle="--", linewidth=1 , color = 'w', label=r"$\nu_{\max}$")
    #plt.axhline(y=ymax, color="m", linestyle="--", linewidth=1)
    #plt.axvline(v_nq, label = 'v nq', linestyle ="--", color = 'w')
    ...
    plt.ylim(bottom = 0.1, top = 2*10**5)
    plt.xlim(left = 1, right = 10**3)
...
star_list = list_import("50_stars_NSR_below_20_test.csv")

def list_stuff_leo(star_list):
    
    for i in range(1, len(star_list)):
        mystar = star_list[i]
        star_name = f"{mystar[0]}: M = {mystar[1]}, R = {mystar[2]}"
        sun_granulation_tau, sun_granulation_sigma = 214, 62.4
        cadence = 25 # cadence of pluto in sconds 
        v_nq = 10**6 / (2 * cadence)  #nyquist frequency in uHz
        ...
        #        wr.writerow([star.freq_powerspectrum_uHz[i], granulation_component[i] + facule_component[i] + oscillation_component[i]])

list_stuff_leo(star_list)
"""
